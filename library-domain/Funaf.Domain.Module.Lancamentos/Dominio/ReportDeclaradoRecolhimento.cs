...
{
    [DBMappedClass(Table = "ReportDeclaradoArrecadacao", PrimaryKey = "idDados")]
    public class ReportDeclaradoRecolhimento
    {
        ...
        public static string CriarPDF(List<ReportDeclaradoRecolhimento> dados, string subTitulo)
        {
            ...
            writer.PageEvent = new HeaderAndFooter("EXTRATO DECLARAÇÃO DOS VALORES DO FUNAF EM "+ dados[0].nuAno, subTitulo, " ");
            doc.Open();

            doc.Add(Chunk.NEWLINE);

            PdfPTable tabela = new PdfPTable(4) { WidthPercentage = 70 };
            tabela.SpacingBefore = 20;
            tabela.SpacingAfter = 20;
            tabela.SetTotalWidth(new float[] { 20,20,20,20 });
            ...
            tabela.DefaultCell.HorizontalAlignment = Element.ALIGN_RIGHT;
            tabela.AddCell(new Phrase("VR PAGO(R$)", FontNegritoHeader));
            tabela.DefaultCell.HorizontalAlignment = Element.ALIGN_RIGHT;
            tabela.AddCell(new Phrase("PAGAMENTO", FontNegritoHeader));
